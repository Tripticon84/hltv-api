...
import {
    GetTeamStatsArguments,
    FullTeamStats
} from '../types/getTeamStats';
...
export const getTeamStats =
    (config: HLTVConfig) =>
        async (options: GetTeamStatsArguments): Promise<FullTeamStats> => {
            ...
            const standins = getPlayersByContainer(
                getContainerByText($, 'Standins')
            )
            ...
            const [m$, e$, mp$] = await Promise.all([
                fetchPage(
                    options.currentRosterOnly
                        ...
                        : `https://www.hltv.org/stats/teams/matches/${options.id
                ...
                fetchPage(
                    options.currentRosterOnly
                        ...
                        : `https://www.hltv.org/stats/teams/events/${options.id
                        }/${generateRandomSuffix()}?${query}`,
                    config.loadPage
                ).then(HLTVScraper),
                fetchPage(
                    options.currentRosterOnly
                        ...
                        : `https://www.hltv.org/stats/teams/maps/${options.id
                        }/${generateRandomSuffix()}?${query}`,
                    config.loadPage
                ).then(HLTVScraper)
            ])
                    ...
                    (stats, mapEl) => {
                        ...
                        stats[mapName] = {
                            ...
                            roundWinPAfterFirstKill: Number(
                                getMapStat(mapEl, 3).split('%')[0]
                            ),
                            roundWinPAfterFirstDeath: Number(
                                getMapStat(mapEl, 4).split('%')[0]
                            )
                        }
