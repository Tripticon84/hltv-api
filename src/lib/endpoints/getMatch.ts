...
import {
    Player
} from '../types/shared/Player'
import {
    fromFullMatchFormat,
    MatchFormatLocation
} from '../types/shared/MatchFormat'
import {
    Demo,
    FullMatch,
    FullMatchTeam,
...
function getTeam($: HLTVPage, n: 1 | 2): FullMatchTeam | undefined {
    return $(`.team${n}-gradient`).exists() ? {
        ...
        id: $(`.team${n}-gradient a`).attrThen('href', (href) => href ? getIdAt(2, href) : undefined),
        rank: $('.teamRanking a').eq(n - 1).contents().eq(1).textThen((x) => parseNumber(x.replace(/#/g, '')))
    } : undefined;
};
...
function getVetoes($: HLTVPage, team1?: Team, team2?: Team): Veto[] {
    ...
    if ($('.veto-box').length > 1) {
        return $('.veto-box').last().find('.padding div').toArray().map((el) => getVeto(el.text()));
    };
...
function getOdds($: HLTVPage): ProviderOdds[] {
    return $('tr.provider:not(.hidden)').toArray().filter((el) => el.find('.noOdds').length === 0).map((oddElement) => {
        const convertOdds = oddElement.find('.odds-cell').first().text().indexOf('%') >= 0;
        ...
        return {
            provider: oddElement
                ...
                .attr('title'),
            team1: convertOdds
                ? percentageToDecimalOdd(oddTeam1)
                : oddTeam1,
...
function getStreams($: HLTVPage): Stream[] {
    return $('.stream-box')
        ...
        .map((streamEl) => ({
            ...
            viewers:
                streamEl.find('.viewers.gtSmartphone-only').numFromText() ?? -1
        }))
        .concat(
    ...
    return $('[class="stream-box"]:not(:has(.stream-box-embed))')
        ...
        .map(function (demoEl) {
            if (demoEl.attr('data-demo-link')) {
                return {
                    name: 'GOTV Demo',
                    link: demoEl.attr('data-demo-link')
                }
            }
